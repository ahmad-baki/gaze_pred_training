...
if __name__ == "__main__":
    ...
    for task in SOURCE_PAR_DIR.iterdir():
        ...
        for trajectory in task.iterdir():
            ...
            # 1) Add language instruction to traj
            success = add_language_instruction(TARGET_DIR / task.name / trajectory.name, task.name)
            if not success:
                faulty_trajectories.append(src_dir)
                continue

            # 2) ALWAYS resize/crop images & depth (overwrites). Also learn if cropping was needed.
            ...
            # 3) Build gaze vectors with x adjusted iff cropping applied
            success = crop_gaze(
                src_dir=src_dir,
                tar_dir=target_dir,
